...
class MicroPostController
{
    ...
    public function index(TokenStorageInterface $tokenStorage, UserRepository $userRepository)
    {
        ...
        if ($currentUser instanceof User) {
            $posts = $this->microPostRepository->findAllByUsers($currentUser->getFollowings());
            $usersToFollow = count($posts) === 0 ? $userRepository->findAllWithPostsExceptUser($currentUser) : [];
        } else {
